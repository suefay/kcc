...
func (ec *Client) SubscribePendingTransactionsEx(ctx context.Context, ch chan<- *types.Transaction, crit filters.PendingTransactionsFilterCriteria) (*rpc.ClientSubscription, error) {
	return ec.c.EthSubscribe(ctx, ch, "newPendingTransactionsEx", crit, false)
}
...
// CallContractEx extends CallContract to retrieve debug trace and logs in the evm call
// debug indicates whether evm debug mode is enabled;
// needLogs indicates whether logs generated by evm call are returned
func (ec *Client) CallContractEx(ctx context.Context, msg ethereum.CallMsg, blockNumber *big.Int, debug bool, needLogs bool) (*ethapi.ExecutionResultEx, error) {
	var result *ethapi.ExecutionResultEx
	err := ec.c.CallContext(
		ctx, &result, "eth_callEx", toCallArg(msg),
		toBlockNumArg(blockNumber), nil, debug, needLogs,
	)
